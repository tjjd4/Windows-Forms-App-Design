...
namespace CourseSystem.Tests
{
    public class Robot
    {
        ...
        // test
        public void ClickListBox(string name, string listName)
        {
            var listBox = _driver.FindElementByAccessibilityId(name);
            var course = listBox.FindElementByName(listName);
            while (course.Displayed != true)
            {
                var down = _driver.FindElementByName("下移一行");
                down.Click();
            }
            course.Click();
        }
        ...
        // test
        public void InputKeyById(string name, string text)
        {
            WindowsElement element = _driver.FindElementByAccessibilityId(name);
        ...
        // test
        public void SelectComboBox(string name, string comboName)
        {
            WindowsElement element = _driver.FindElementByAccessibilityId(name);
            element.Click();
            element.FindElementByName(comboName).Click();
        }

        // test
        public void AssertDataGridViewData(string name, int rowIndex, string columnName, string except)
        {
            var dataGridView = _driver.FindElementByAccessibilityId(name);
            var element = _driver.FindElementByName(columnName.ToString() + " 資料列 " + rowIndex.ToString());
            Assert.AreEqual(except, element.Text);
        }

        // test
        public void ClickButtonPrecision(string formname, string name)
        {
            var elements = _driver.FindElementByAccessibilityId(formname).FindElementsByXPath("//*");
            foreach (var element in elements)
            {
                if (("ControlType.Button" == element.TagName) && (name == element.Text))
                {
                    element.Click();
                }
            }
