...
namespace DrawingApp
{
    ...
    sealed partial class App : Application
    {
        ...
        protected override void OnLaunched(LaunchActivatedEventArgs e)
        {
            ...
            if (rootFrame == null)// 當視窗中已有內容時，不重複應用程式初始化，只確定視窗是作用中
            {
                rootFrame = new Frame();// 建立框架做為巡覽內容，並巡覽至第一頁
            ...
            if (e.PrelaunchActivated == false)
            {
                if (rootFrame.Content == null)
                    rootFrame.Navigate(typeof(MainPage), e.Arguments);// 在巡覽堆疊未還原時，巡覽至第一頁，設定新的頁面，方式是透過傳遞必要資訊做為巡覽參數
